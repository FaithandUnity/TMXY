...
param(
    ...
    [string]$OutputPath = 'E:\QQXYCodeDev\Rebuild\Data\BuildBaseline\p1-06-package-v3.json'
)
...
function Test-PackageV3Header {
    param([Parameter(Mandatory = $true)][string]$Path)
    $version = [System.Text.Encoding]::ASCII.GetBytes('QRENDER PACKAGE VER 3.0')
...
$requiredFiles = @(
    ...
    'Tools/TMXY.Package/src/package_v3.cpp',
...
foreach ($relativePath in $requiredFiles) {
    if (-not (Test-Path -LiteralPath (Join-Path $root $relativePath) -PathType Leaf)) {
        throw "P1-06 required file is missing: $relativePath"
    }
}
...
foreach ($file in $sourceFiles) {
    if ((Get-Content -LiteralPath $file.FullName -Raw -Encoding UTF8) -match $forbiddenPattern) {
        throw "P1-06 forbidden legacy/platform dependency found: $($file.FullName)"
    }
}
...
if ($manifestSha -ne $expectedManifestSha) {
    throw 'P1-06 requires the frozen client 3.0.0.413 raw manifest.'
}
...
foreach ($file in Get-ChildItem -LiteralPath $packagesRoot -Recurse -File | Sort-Object FullName) {
    if (-not (Test-PackageV3Header -Path $file.FullName)) { continue }
    ...
    if (-not $manifestByPath.ContainsKey($relative)) {
        throw "P1-06 Package 3.0 sample is absent from the frozen manifest: $relative"
    }
    ...
    if ($file.Length -ne [int64]$manifestRecord.size -or
        ...
        throw "P1-06 Package 3.0 sample differs from the frozen manifest: $relative"
    }
...
$expectedSampleSetSha = '9e6872a12464bcbe7c98de2cd96a9ad3d831156a14a7b50b1a2dd07ec75260dd'
$sampleBytes = ($sampleRecords | Measure-Object -Property size -Sum).Sum
if ($sampleRecords.Count -ne 140 -or $sampleBytes -ne 36333329 -or
    $sampleSetSha -ne $expectedSampleSetSha) {
    throw 'P1-06 Package 3.0 sample set differs from the frozen 140-file baseline.'
}
...
if (-not $imagePresent -or $actualBuilderId -ne $expectedBuilderId -or $builderUser -ne 'tmxy') {
    throw 'P1-06 requires the qualified non-root Clang 21 builder image.'
}
...
./out/build/ci-linux-clang/TMXY.Package/tests/tmxy_package_v3_reader_test /legacy/Packages
'@
...
$passed = $execution.exit_code -eq 0 -and $execution.output -match '100% tests passed' -and
    $null -ne $sampleResult -and [int]$sampleResult.sample_count -eq 140 -and
    [int]$sampleResult.record_count -eq 91074 -and
    [int64]$sampleResult.directory_bytes -eq 4143928 -and
    [int64]$sampleResult.file_bytes -eq 36333329 -and
    [string]$sampleResult.aggregate_fnv1a64 -eq 'f8ec20e8fff7b975'
...
$report = [pscustomobject][ordered]@{
    ...
    transform = [pscustomobject][ordered]@{
        full_group = '~E2,~E3,E0,~E1'
        remainder_1 = 'E0'
        remainder_2 = 'E0,~E1'
        remainder_3 = '~E2,E0,~E1'
    }
    container_structure = [pscustomobject][ordered]@{
        outer_header_bytes = 29
        decoded_prefix_hex = '030076657264000000010000003f'
        independent_block_table_observed = $false
        embedded_container_checksum_observed = $false
        body_layout = 'record-declared-contiguous-ranges-to-eof'
        integrity_authority = 'frozen-manifest-sha256'
    }
...
$json
if (-not $passed) { throw "P1-06 locked-builder validation failed: $($execution.output)" }
