...
inline constexpr std::string_view kPackageV3Version = "QRENDER PACKAGE VER 3.0";
inline constexpr std::array<std::byte, 14> kPackageV3DirectoryPrefix{
...
enum class PackageV3ErrorCode : std::uint8_t
{
    read_failure = 1,
    invalid_version = 2,
...
struct PackageV3ObjectRecord final
{
...
struct PackageV3Header final
{
    std::string version;
    std::vector<PackageV3ObjectRecord> records;
...
[[nodiscard]] std::string_view to_string(PackageV3ErrorCode code) noexcept;
[[nodiscard]] std::uint64_t package_v3_metadata_fingerprint(const PackageV3Header& header) noexcept;
