...
P1-10 已用钥匙链中的 P1-09 候选在内存中解开当前表并完成逐行关系证明。两份载荷
都是严格 GBK，表头逐 byte 相同且均为 95 列；当前表有 29,223 行，残留 CSV 有
27,289 行。首字段在两边均为唯一十进制主键：27,288 个主键共有，其中 26,272 行
逐 byte 相同、1,016 行内容变化；当前表新增 1,935 个主键，残留侧独有 1 个。载荷
共同前缀为 13,769 bytes，按位置第 60 行首次不同，全部相同行位置只有 916 个。

机器报告为 `Data/BuildBaseline/p1-10-current-item-csv-relation.json`，生成器为
`Tools/TMXY.Table/Compare-CurrentItemResidual.ps1`。报告不含字段、主键、明文或 key。
因此 P1-10 完成；残留 CSV 只能用于 Schema 和历史行差异证据，不能覆盖当前解密
结果，也不能充当发布资产。
...
P1-11 已固定活动 `CLSVShare` 中 10 张最小表和 10 张最大表，合计 21,064,736
ciphertext bytes。20/20 均通过双 AES、padding、无 NUL、CRLF、编码和固定列数检查；
11 张为纯 ASCII，9 张为严格 GBK。所有样本都记录行数、表头列数、列数范围、空字段
总数和含空值行数。

首字段检查没有把字段名或值写入报告：15 张可作为唯一首字段候选，5 张明确标记为
需要复合键。大型样本中，`item_table` 为 29,223 行/95 列且首字段唯一；
`skill_table` 为 23,227 行/65 列，首字段只有 9,278 个唯一值、13,949 次重复，因此
不能把首字段误当单列主键。20 张样本均无解密或结构失败。

生成器为 `Tools/TMXY.Table/Inspect-CurrentTableRepresentativeSet.ps1`，机器报告为
`Data/BuildBaseline/p1-11-current-table-representatives.json`，无 Secret/明文/标识符输出。
至此 P1-09～P1-11 均完成，下一项为 P1-28/G1 格式评审。
