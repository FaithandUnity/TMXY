...
foreach ($line in [System.IO.File]::ReadLines($graph)) {
    ...
    $matches = if ($queryNodeId.Length -gt 0) {
        ...
        $line.Contains($queryNameHash, [System.StringComparison]::Ordinal)
    }
    if (-not $matches) { continue }
    $entry = $line | ConvertFrom-Json
    if (($queryNodeId.Length -gt 0 -and [string]$entry.id -eq $queryNodeId) -or
        ($queryNameHash.Length -gt 0 -and
            [string]$entry.logical_name_sha256 -eq $queryNameHash)) {
        $nodes.Add($entry)
    }
}
...
foreach ($line in [System.IO.File]::ReadLines($graph)) {
    ...
    $entry = $line | ConvertFrom-Json
    if ($possibleIncoming -and [string]$entry.target_logical_name_sha256 -eq $queryNameHash) {
