...
和未知为零，合同拒绝首候选、明细遗漏/重复、矛盾状态及伪造归零。核心闭包还独立重开并
精确哈希验证 P2-20A.3 报告、Policy 与 Schema；212 个配置实例、171/35/6 非终态和零批准根
会继续传递到 G2，而不能因存在词法候选就被提升为语义闭包。
...
`Contract/Test-G2AuxEcfParserParity.ps1` 验证 P2-20A.10 的三层 ECF 诊断：A.5 历史基线保持
61/64，冻结 A.3 输出对源码派生旧版参考为 51/64，同一正确明文上的 A.3 过滤器对旧版成对
解析器为 63/64。合同逐行验证 64 条匿名明细、3 字节尾段、候选投影和披露边界，并拒绝把
源码派生端口当作旧版运行时执行、二进制等价、有效语义导入或任何 G2/P3 授权。

P2-20A.11 malformed XML 合同验证四层诊断：
诊断：两个严格层均拒绝 6/6 个输入，TinyXML 2.3.4 源码派生 `LoadFile` 接口接受 6/6，
